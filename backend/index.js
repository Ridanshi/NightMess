...
app.post("/confirm_order", async (req, resp) => {
  try {
    if (req.session.isLoggedIn) {
      ...
      const allCartItems = await Cart.find({ client_email: clientEmail });
      const cartItemsWithFood = await Promise.all(
        allCartItems.map(async (item) => ({
          item,
          food: await FoodData.findById(item.foodId)
        }))
      );
      const cartItems = cartItemsWithFood
        .filter(({ food }) => food && food.vendor_email === selectedVendorEmail)
        .map(({ item }) => item);
      ...
      const walletUpdate = await ClientData.updateOne(
        {
        ...
        {
          $inc: { "mess_wallets.$.balance": -totalAmount }
        }
      );
...
app.post("/request_recharge", async (req, res) => {
  try {
    ...
    if (!Number.isFinite(requestAmount) || requestAmount <= 0) {
      return res.status(400).json({ success: false, msg: "Enter a valid recharge amount" });
    }
  ...
  try {
    if (req.session.isLoggedIn && req.session.usertype === 'vendor') {
      ...
      if (request.vendor_email !== vendorEmail) {
        return res.status(403).json({
          success: false,
          msg: "You cannot reject requests for other messes"
        });
      }
